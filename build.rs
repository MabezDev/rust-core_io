...
fn parse_mappings(mut mappings: &'static str) -> Vec<Mapping> {
    ...
    let mut result =
        Vec::with_capacity(mappings.len() / (P1.len() + 40 + P2.len() + 40 + P3.len()));
...
const CONDITIONAL_CFGS: &'static [ConditionalCfg] = &[
    ...
    (
        Some("rustc_unicode"),
        &[("2016-12-15", Some("std_unicode")), ("2017-03-03", None)],
    ),
];
...
fn main() {
    ...
    let io_commit = match env::var("CORE_IO_COMMIT") {
        ...
        Err(env::VarError::NotPresent) => {
            ...
            let compiler = ver
                .commit_hash
                .expect("Couldn't determine compiler version");
            mappings
                .iter()
                .find(|&&Mapping(elem, _)| elem == compiler)
                .expect("Unknown compiler version, upgrade core_io?")
                .1
                .to_owned()
        }
    };
